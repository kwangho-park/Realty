package kr.com.pkh.batch.extend.job.confitionalStep;
    ...
    public Job conditionalStepJob(
        ...
        return jobBuilderFactory.get("conditionalStepJob")
                ...
                .start(conditionalStartStep)
                    .on("FAILED").to(conditionalFailStep)   // conditionalStartStep (Step) 이 실패인 경우 conditionalFailStep (Step) 실행
                .from(conditionalStartStep)
                    .on("COMPLETED").to(conditionalCompletedStep)   // conditionalStartStep (Step) 이 성공인 경우
                .from(conditionalStartStep)
                    .on("*").to(conditionalAllStep)         // conditionalStartStep (Step) 그외의 결과인 경우
                .end()
                .build();
    }
