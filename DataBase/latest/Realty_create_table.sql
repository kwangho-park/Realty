...
CREATE TABLE IF NOT EXISTS `realty`.`TB_REGION_CODE` (
  `RC_ID` INT(11) NOT NULL AUTO_INCREMENT COMMENT '시퀀스',
...
CREATE TABLE IF NOT EXISTS realty.TB_USER_INFO (
    `UI_ID` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '시퀀스',
...
CREATE TABLE IF NOT EXISTS realty.TB_USER_LOG (
  `UL_ID` INT NOT NULL AUTO_INCREMENT COMMENT '시퀀스',
...
-- 아파트 건물 면적타입 테이블 
CREATE TABLE IF NOT EXISTS realty.TB_AREA_TYPE (
  `AT_NUM` INT NOT NULL AUTO_INCREMENT COMMENT '시퀀스',
  `AT_PRIVATE_AREA` VARCHAR(45) NULL COMMENT '주거전용면적',
  ...
  CONSTRAINT `fk_TB_AREA_TYPE_TB_APT_BUILDING`
    FOREIGN KEY (`AT_AB_PNU`)
    REFERENCES realty.TB_APT_BUILDING (`AB_PNU`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '아파트 면적 타입 테이블';