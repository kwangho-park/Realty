...
CREATE TABLE IF NOT EXISTS `realty`.`TB_REALTY_INFO` (
  ...
  `RI_INSERT_DATE` DATETIME NULL COMMENT '등록일자',
  `RI_UPDATE_DATE` DATETIME NULL COMMENT '수정일자',
...
CREATE TABLE IF NOT EXISTS `realty`.`TB_REGION_CODE` (
  ...
  `RC_REGION_CODE` INT(10) UNSIGNED NULL COMMENT '지역코드 (10자리)',
  `RC_SIDO_CODE` INT(2) UNSIGNED NULL COMMENT '시도코드 (2자리)',
  `RC_SGG_CODE` INT(3) UNSIGNED NULL COMMENT '시군구코드 (3자리) ',
  `RC_UMD_CODE` INT(3) UNSIGNED NULL COMMENT '읍면동 코드 (3자리) ',
  `RC_RI_CODE` INT(2) UNSIGNED NULL COMMENT '리 코드 (2자리)',
